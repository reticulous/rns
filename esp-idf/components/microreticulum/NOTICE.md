...
| `src/Interface.h` | ArduinoJson include + adapters dropped (same reason as `Bytes.h`). Added `announce_cap` and `point_to_point` accessors and the `_point_to_point` member (hidden-node-aware split horizon, see `rns/INTERNALS.md` §1.1.1). |
| `src/Interface.cpp` | Implemented `process_announce_queue()` (was a stub): drains the bandwidth-cap hold queue, lowest hop count first, driven from `Transport::jobs()`. |
| `src/Transport.h` | Declared `_discovery_pr_tags_order` (FIFO eviction companion to `_discovery_pr_tags`). |
...
| `src/Link.{cpp,h}`, `src/Packet.h`, `src/Type.h` | Selected post-pin upstream improvements backported from `attermann/microReticulum` `032c751` (0.5.0 era): initiator-side per-hop establishment-timeout grace (upstream commit `35bb40e`, contributed by @ronandi) so a multi-hop link doesn't time out mid-establishment; per-packet link signal-quality capture (`Link::rssi()/snr()/q()`, fed from new `Packet` rssi/snr/q accessors that default to `Type::NaN` — dormant until an interface populates them); and snapshot-before-cancel iteration over the resource sets in `Link::link_closed()` and the RESOURCE/HMU/PRF branches of `Link::receive()` (a concluding callback can erase mid-iteration). `Type.h` gains the `NaN`/`isNan` helpers the capture's availability check needs. |
| `src/Resource.{h,cpp}`, `src/ResourceData.h` | Resource transfer engine implemented; the algorithm ancestry is `ratspeak/microReticulum` (© ratspeak, Apache-2.0), adapted to this fork's APIs and corrected to upstream Reticulum's proof/hash wire format where the two diverge. Upstream's attermann-shaped `Resource`/`ResourceData`/`ResourceAdvertisement` API surface is preserved. In-file attribution carried in `Resource.h`. RESOURCE_REQ packet-hash dedupe (`_req_hashlist` / `has_request_hash`/`note_request_hash`) backported from `032c751` so a retransmitted request no longer resends the whole part window; upstream keeps the list unbounded, this fork bounds it (retransmits only ever arrive within a few RTTs). |
